...
Do not apply the manifest until the image digest, Secret Manager integration, ingress, network policy, and durable queue are configured. The future worker Job must inject `ADK_API_KEY`, `ADK_BASE_URL`, and any required `ADK_AUTHORIZATION`, `ADK_CLIENT_ID`, and `ADK_CLIENT_SECRET` from Secret Manager; it must not reference desktop files.

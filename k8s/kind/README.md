...
The manifest enables the full local flow: `WORKER_ENABLED=true` and `PUBLISHING_ENABLED=true`. OpenHands always runs in its own Kubernetes Job. `openhands-smoke-job.yaml` separately proves the published OpenHands worker image starts and completes as a Kubernetes Job.
...
For a Gemini run, the Secret named by `OPENHANDS_KUBERNETES_SECRET_NAME` must contain both `openhands-api-key` and `gemini-api-key`. They hold the same Gemini connector key: the OpenHands SDK receives the first key and the OpenHands image uses the second for its bundled compatibility bridge. Java references the Secret by name but never reads or copies its values; create it through your local setup or Helm/External Secrets process.
